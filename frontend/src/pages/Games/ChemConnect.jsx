...
const ChemConnect = () => {
  ...
  const questions = [
    { q: "Which element has atomic number 8?", options: ["Nitrogen", "Oxygen", "Fluorine"], answer: "Oxygen" },
    { q: "Which element symbol is 'Na'?", options: ["Neon", "Sodium", "Nitrogen"], answer: "Sodium" },
    { q: "Calcium belongs to which group?", options: ["Alkali Metals", "Alkaline Earth Metals", "Noble Gases"], answer: "Alkaline Earth Metals" },
    { q: "Which element is a noble gas?", options: ["Neon", "Sodium", "Nitrogen"], answer: "Neon" },
  ];
    ...
    <div className="flex flex-col items-center min-h-screen bg-gradient-to-r from-blue-100 to-indigo-100 p-6">
      <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-4xl text-center">
        ...
        <div className="grid grid-cols-10 gap-2 justify-center mb-8">
          {elements.map((el) => (
            <div key={el.num} className="border rounded-md p-2 text-center text-sm bg-indigo-50 hover:bg-indigo-200 transition">
        ...
        {!gameOver ? (
          <>
            <h3 className="text-lg font-semibold text-gray-800 mb-4">{questions[currentQ].q}</h3>
            <div className="flex flex-col gap-3 mb-6">
              {questions[currentQ].options.map((opt, i) => (
                <button
                  key={i}
                  onClick={() => setSelected(opt)}
                  className={`px-4 py-2 rounded-lg border ${selected === opt ? 'bg-indigo-500 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}
                >
                  {opt}
                </button>
              ))}
          ...
          <>
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Quiz Complete! 🎉</h3>
            <p className="text-lg mb-6">
              Your Score: <span className="font-bold text-indigo-600">{score}</span> / {questions.length}
            </p>
            <button onClick={resetGame} className="px-6 py-2 bg-indigo-600 text-white rounded-xl shadow hover:bg-indigo-700 transition mb-4">
              Play Again
            </button>
          </>
        )}
